...
class ConvertToTxt:
    ...
    def __init__(self, directory_path=None, file=None, thread_limit=5, base_path=None):
        """
        Constructor for the ConvertToTxt class.

        :param directory_path: The path of the directory where the files are to be saved.
        ...
        self.__get_medical_words()
        self.words = set(nltk.corpus.words.words())

        try:
            if isinstance(file, str):
                self.folder_structure = "".join(file.replace(self.base_path, '').rsplit('\\', 1)[:-1]).replace("\\", "")
                os.makedirs(os.path.join(self.folder_paths['input_path'], self.folder_structure), exist_ok=True)
                shutil.copy(file, os.path.join(self.folder_paths['input_path'], self.folder_structure))
            else:
                file.save(os.path.join(self.folder_paths['input_path'], file.filename))
        except Exception as e:
            print("__init__:", e)
            shutil.move(file, os.path.join(self.folder_paths['input_path'], file.rsplit('/', 1)[-1]))
    ...
    def get_files_by_name_or_count(self, file_name=None, count=None):
        """
        ...
        :param count: Optional. The number of files to process.
        :return: Returns a list of files found in the folder path.
        """
        if file_name is None:
            files = self.__get_files_from_directory(count=count)
        else:
            files = self.__get_files_from_directory(file_name=file_name)
        return files

    def __get_medical_words(self):
        self.ocr_db = mysql.connector.connect(
            host="digital.quicsolv.com",
            port=33400,
            user="digitizeme",
            passwd="8xBt3Gja84I&%!Vk_*pP%Fk_K4s63_hsL",
            database="digitizeme",
            auth_plugin='mysql_native_password'
        )
        mycursor = self.ocr_db.cursor()
        mycursor.execute("SELECT * FROM default_keywords WHERE kt_id IN(2, 3, 4) ORDER BY kw_name ASC")
        myresult = mycursor.fetchall()
        for test in myresult:
            self.medical_words.append(test[1].lower())
    ...
    def __get_files_from_directory(self, file_name=None, count=None):
        if file_name is None:
    ...
    def __normalize_image_to_scale(self, img_path=None, img=None, required_width=1600, required_height=2250):
        ...
        self.__dims = int(og_width * scale_width), int(og_height * scale_height)
        return cv2.resize(img, self.__dims, interpolation=cv2.INTER_AREA)
    ...
    @staticmethod
    def validate_file(key, file_name):
        return True
        # try:
        #     response = requests.post('http://' + ':'.join(
        #         [configuration.get('qs_server', 'url'), configuration.get('qs_server', 'port')]) + "/process_report",
        #                              data=json.dumps(dict(license_key=key, file_name=file_name)))
        #     if response.json()['file_name'] == file_name and response.json()['allowed_to_process']:
        #         return True
        #     else:
        #         return False
        # except:
    ...
    def process_files(self, files, dpi=900, save_to_txt=False, return_page_count=False, move_file=False):
        ...
        for file in files:
            ...
            try:
                is_valid = self.validate_file(key, file.rsplit('/', 1)[-1])
                if not is_valid:
                    raise PermissionError(
                        "Cannot process file '{0}'. Please remove it from the processing queue and then proceed.".format(
                            file.rsplit('/', 1)[-1]))
                ...
                phase_two_text_dict, time_dict = self.__extract_from_pdf_b(file, dpi)
                # phase_two_text_dict, time_dict = {}, {}
                phase_one_text_dict = {int(k) + 1: v for k, v in phase_one_text_dict.items()}
                phase_two_text_dict = {int(k): v for k, v in phase_two_text_dict.items()}
                ...
                for k, v in sorted(counter_dict.items(), key=lambda item: item[0]):
                    ...
                    if k in phase_one_text_dict and k in phase_two_text_dict:
                        ...
                        # print("English %:")
                        # print("Phase 1:", one_eng_percent)
                        # print("Phase 2:", two_eng_percent)
                        ...
                        if one_eng_percent >= two_eng_percent:
                            ...
                            # print("Medical %:")
                            # print("Phase 1:", one_med_percent)
                            # print("Phase 2:", two_med_percent)
        ...
        if return_page_count:
            return self.__filename, final_text_dict, page_count, self.mer_output
        else:
            return self.__filename, final_text_dict, self.mer_output
    ...
    def __extract_from_pdf_b(self, file_path, dpi):
        ...
        if file_path.rsplit('.', 1)[-1].lower() == 'pdf':
            temp_path = os.path.join(os.getcwd(), "extracted_png")
    ...
    def __perform_ocr(self, args):
        ...
        large = self.__normalize_image_to_scale(img=large, required_width=1600 * 4, required_height=2250 * 4)
        try:
            rot_data = pytesseract.image_to_osd(large, output_type=Output.DICT)
            ...
            if angle > 0:
                large = ndimage.rotate(large, angle)
        except Exception as e:
            print(e)
        # large = cv2.cvtColor(large, cv2.COLOR_GRAY2BGR)

        large = self.__normalize_image_to_scale(img=large)
        large = cv2.cvtColor(large, cv2.COLOR_GRAY2BGR)
        image = large.copy()

        img_rotated = self.__rotate_img(image, page_idx)

        cv2.imwrite(os.path.join(self.extracted_pages_path, self.__filename, self.__filename.rsplit('/', 1)[-1] + '-' + str(page_idx) + ".png"), img_rotated)
        ...
        # cv2.waitKey()
        boxes = self.__get_boxes(small, result, page_idx)
    ...
    @staticmethod
    def __get_boxes(og_image, result, page_idx):
        ...
        connected = cv2.morphologyEx(bw.copy(), cv2.MORPH_CLOSE, kernel)
    ...
    def df_to_text(self, img_block, page_no):
    ...
    def __create_lines(self, line_items, page_image, boxes, page_no):
        ...
        for box in boxes:
            if len(line_items) > 0:
                if abs(line_items[-1].line_coordinates[1] - box[1]) < 8:
                    ...
                    box_item.text = self.df_to_text(page_image[max(box[1] - padding, 0):min(box[3] + padding, page_image.shape[0]), max(box[0] - padding, 0):min(box[2] + padding, page_image.shape[1])], page_no)
        ...
        os.makedirs(os.path.join(self.extracted_pages_path, self.__filename + '_ocr'), exist_ok=True)
        cv2.imwrite(
            os.path.join(self.extracted_pages_path, self.__filename + '_ocr', self.__filename.rsplit('/', 1)[-1] + '-' + page_no + ".png"),
            display_img)
        return line_items
