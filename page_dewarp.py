...
OUTPUT_DPI = 450  # just affects stated DPI of PNG, not appearance
...
def optimize_params(name, small, dstpoints, span_counts, params):
    ...
    res = optimize.minimize(objective, params, method='Powell')
    params = res.x
...
def get_page_dims(corners, rough_dims, params):
    ...
    res = optimize.minimize(objective, dims, method='Powell')
    dims = res.x
...
#         line = linecache.getline("page_dewarp.py", lineno)
#         end = time.time()
#         global start
#         print("Line {0}:\t{1}\t{2}".format(lineno, end - start, line.rstrip()))
#         start = time.time()
#     return traceit
#
#
